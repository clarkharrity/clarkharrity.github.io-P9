...
// — Sample Product Array (truncate or expand as needed) —
const allProducts = [
  {
    id: 1,
    brand: "CeraVe",
    ...
    description: "Gentle gel cleanser with ceramides, hyaluronic acid, and niacinamide. Removes oil, dirt, and makeup without harming the barrier."
  },
  {
    id: 2,
    brand: "CeraVe",
    ...
    description: "Non-foaming cleanser for normal to dry skin. Includes ceramides and hyaluronic acid to lock in moisture."
  },
  {
    "id": 3,
    "brand": "CeraVe",
  ...
  },
  {
    ...
    "description": "Mascara that instantly builds 5 times fuller lashes with panthenol and ceramide-R. Exclusive brush coats and separates without clumping. Rich Carbon Black pigment for dramatic depth. Ophthalmologist-tested, fragrance-free; soft, plush lashes removable with soap and water."
  },
  {
    "id": 18,
    "brand": "L'Oréal Paris",
...
function displayProducts(category) {
  ...
  productsContainer.innerHTML = filtered.map(p => `
    <div class="product-card ${selectedProducts.includes(p.id) ? 'selected' : ''}" data-id="${p.id}">
      <img src="${p.image}" alt="${p.name}">
      <div class="product-info">
        <h3>${p.name}</h3>
        <p>${p.brand}</p>
        <button class="desc-toggle" onclick="toggleDescription(${p.id})">Show Description</button>
        <p id="desc-${p.id}" class="product-desc hidden">${p.description}</p>
      </div>
    </div>
  `).join("");
...
function updateSelectedList() {
  const sel = allProducts.filter(p => selectedProducts.includes(p.id));
...
generateBtn.addEventListener("click", async () => {
  ...
  chatWindow.innerHTML += `<div><strong>You:</strong> Generate a routine based on selected products</div>`;

  const response = await fetch("https://wonderbot.ufh86.workers.dev", {
    method: "POST",
  ...
  chatWindow.innerHTML += `<div class="chat-response"><strong>AI:</strong><br>${formatted}</div>`;
...
chatForm.addEventListener("submit", async (e) => {
  ...
  if (!text) return;
  chatWindow.innerHTML += `<div><strong>You:</strong> ${text}</div>`;
  input.value = "";
